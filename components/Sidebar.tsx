...
export default function Sidebar() {
  ...
  // Shared Sidebar Content Body
  const SidebarBody = ({ isMobile = false }: { isMobile?: boolean }) => (
    <div className="flex flex-col h-full bg-[#0a0d11] text-zinc-200 font-mono select-none">
      ...
      <div className="p-3.5 sm:p-4 border-b-2 border-zinc-800 flex items-center justify-between gap-2">
        ...
        >
          <SparkleIcon size={30} className="flex-shrink-0 group-hover:scale-110 transition-transform text-[var(--theme-color)]" />
        ...
        {isMobile ? (
          <button
            ...
            className="p-1.5 rounded-md bg-zinc-900 border border-zinc-700 text-zinc-400 hover:text-white cursor-pointer text-xs"
          >
            ✕
          </button>
        ) : (
          <button
            ...
            className="hidden md:flex p-1.5 rounded-md bg-[#12161d] border border-zinc-700 hover:border-white text-zinc-400 hover:text-white cursor-pointer transition-all shadow-[1px_1px_0px_0px_#000000]"
          >
            <svg
      ...
      {/* 2. ChatGPT Style Quick CTA Button */}
      <div className="p-3">
        <Link
          href="/launch"
          onClick={() => isMobile && setIsOpen(false)}
        ...
        >
          ...
          {(!isCollapsed || isMobile) && <span>DEPLOY TOKEN</span>}
        </Link>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 flex flex-col gap-1.5 custom-scrollbar">
        <div className="px-2 py-1 text-[10px] font-black text-zinc-500 uppercase tracking-wider">
          {(!isCollapsed || isMobile) ? '// NAVIGATION' : '// NAV'}
        </div>
      ...
      <div className="p-3 border-t-2 border-zinc-800 bg-[#07090c] flex flex-col gap-2">
        ...
        <div className="relative">
          <button
            ...
            className={`w-full flex items-center gap-2 bg-[#12161d] border border-zinc-700 hover:border-white px-2.5 py-1.5 rounded-md text-xs text-zinc-300 hover:text-white transition-all cursor-pointer ${
        ...
        {isConnected ? (
          <div className="flex flex-col gap-1.5">
            <div className={`flex items-center gap-2 p-2 rounded-lg bg-[#12161e] border border-zinc-800 ${isCollapsed && !isMobile ? 'justify-center p-1.5' : ''}`}>
              <div
            ...
            <button
              ...
              className={`w-full py-1.5 px-2 bg-rose-950/40 hover:bg-rose-900 border border-rose-800/80 hover:border-rose-500 text-rose-300 text-[11px] font-black uppercase rounded transition-all cursor-pointer ${
                isCollapsed && !isMobile ? 'text-[9px] px-0' : ''
              }`}
            >
              {isDisconnecting ? '...' : (!isCollapsed || isMobile ? '[✕] DISCONNECT' : 'EXIT')}
            </button>
          </div>
        ) : (
          <div className="relative">
            <Button
              ...
              className={`w-full text-xs font-black uppercase py-2 shadow-[2px_2px_0px_0px_#ffffff] ${
                isCollapsed && !isMobile ? 'px-1 text-[10px]' : ''
              }`}
            >
              {(!isCollapsed || isMobile) ? 'CONNECT WALLET' : 'LOGIN'}
            </Button>
  ...
  return (
    <>
      {/* A. Desktop Fixed Left Sidebar */}
      <aside
        className={`hidden md:block fixed top-0 bottom-0 left-0 z-30 border-r-2 border-zinc-800 transition-all duration-300 ${
          isCollapsed ? 'w-20' : 'w-64'
        }`}
      >
        <SidebarBody isMobile={false} />
      </aside>

      {/* B. Mobile Drawer (Overlay + Slide-in Sidebar) */}
      {isOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          <div className="relative w-4/5 max-w-xs h-full z-10 border-r-2 border-zinc-700 shadow-2xl animate-slideIn">
            <SidebarBody isMobile={true} />
          </div>
        </div>
      )}
    </>
